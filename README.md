...
2. **触发**：内置定时任务每分钟检查一次，到达 `report_time`（默认 23:00）后，对当天有消息的每个群生成日报并主动推送；也可以在群里发送 **「兔兔今日日报」** 立即手动触发本群日报
3. **生成**：
...
| `report_group_whitelist` | 群聊白名单，只给这些群发送日报；留空表示所有群都发送 | `[]` |
| `report_group_blacklist` | 群聊黑名单，这些群不发送日报；黑名单优先于白名单 | `[]` |
...
在任意群聊里发送 **`兔兔今日日报`**（也可以带空格：`兔兔 今日日报`），即可立即为当前群生成并推送今天的日报，不用等定时到点。

- 白名单/黑名单同样对手动触发生效
- 当天还没有任何消息记录时会提示「今天还没有任何消息记录」
- 机器人自己的转发消息不会触发指令
